...
class LaserScanMatcherOdometry {
 private:
  ...
  // subscriber
  ros::Subscriber scan_subscriber_, cloud_subscriber_, odom_subscriber_,
      imu_subscriber_, vel_subscriber_;
  ...
  tf::Transform
      laser_to_base_;  // static, cached, calculated from base_to_laser_
  ...
  void getPrediction(double& pr_ch_x, double& pr_ch_y, double& pr_ch_a,
                     double dt);
