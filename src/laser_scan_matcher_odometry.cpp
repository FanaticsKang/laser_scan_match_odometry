...
LaserScanMatcherOdometry::LaserScanMatcherOdometry(ros::NodeHandle nh,
  ...
  if (use_cloud_input_) {
    cloud_subscriber_ = nh_.subscribe(
        "cloud", 1, &LaserScanMatcherOdometry::cloudCallback, this);

  } else {
    scan_subscriber_ =
        nh_.subscribe("scan", 1, &LaserScanMatcherOdometry::scanCallback, this);
  }

  if (use_imu_) {
    imu_subscriber_ = nh_.subscribe(
        "imu/data", 1, &LaserScanMatcherOdometry::imuCallback, this);
  }

  if (use_odom_) {
    odom_subscriber_ =
        nh_.subscribe("odom", 1, &LaserScanMatcherOdometry::odomCallback, this);
  }
...
void LaserScanMatcherOdometry::initParams() {
  ...
  if (!nh_private_.getParam("use_imu", use_imu_)) use_imu_ = true;
  ...
  if (!nh_private_.getParam("alpha", alpha_)) alpha_ = 1.0;
  if (!nh_private_.getParam("beta", beta_)) beta_ = 0.8;
...
void LaserScanMatcherOdometry::cloudCallback(
...
void LaserScanMatcherOdometry::odomCallback(
...
void LaserScanMatcherOdometry::createCache(
    const sensor_msgs::LaserScan::ConstPtr &scan_msg) {
  a_cos_.clear();
  a_sin_.clear();
...
void LaserScanMatcherOdometry::getPrediction(double &pr_ch_x, double &pr_ch_y,
  ...
  if (use_odom_ && received_odom_) {
    pr_ch_x =
        latest_odom_.pose.pose.position.x - last_odom_.pose.pose.position.x;

    pr_ch_y =
        latest_odom_.pose.pose.position.y - last_odom_.pose.pose.position.y;
...
double LaserScanMatcherOdometry::getYawFromQuaternion(
...
void LaserScanMatcherOdometry::createTfFromXYTheta(double x, double y,
                                                   double theta,
                                                   tf::Transform &t) {
